...
ReportBugs.TaskEncodeImage = cc.Class.extend({
    ctor: function ctor() {
        this.name = "encodeImage";
    ...
    execute: function execute() {
        ...
        if (this.isCoroutine) {
            ...
            if (this.currentIndex >= this.buffer.byteLength) {
                this.progress = 1.0;
                this.isCompleted = true;
    ...
    encode: function encode(buffer) {
        ...
        var endIndex = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : buffer.byteLength;
        ...
        for (var f = startIndex; f <= endIndex && f < buffer.byteLength; f += 3) {
            n = buffer[f + 0];
            r = buffer[f + 1];
            i = buffer[f + 2];
            s = n >> 2;
